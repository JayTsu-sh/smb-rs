...
pub(crate) fn start_generation(
    ...
    tokio::spawn(async move {
        owner_task(transport, clock, config, admission_rx, control_rx, event_tx).await;
...
async fn owner_task(
    ...
    loop {
        ...
        if let Ok(command) = admission_rx.try_recv() {
            process_admission(command, &mut state, &mut terminals, &mut send_queue);
        }
        dispatch_next(
            &write_tx,
            &mut state,
        ...
        tokio::select! {
            ...
            command = control_rx.recv() => match command {
                Some(command) => {
                    close_admission_if(
                        handle_control(command, &mut state, &mut terminals, &mut frame_cancellations, &mut deferred_cancellations, &mut send_queue, &mut close_request),
                        &mut admission_rx,
                    );
                }
            ...
            event = io_rx.recv() => match event {
                Some(event) => {
                    if process_io(event, &mut state, &mut terminals, &mut frame_cancellations, &mut deferred_cancellations, &event_tx, &mut fatal) {
                        admission_rx.close();
                    }
                }
                None => {
            ...
            _ = &mut deadline_sleep => {
                let effects = state.reduce(OwnerEvent::AdvanceTime { now: clock.now() });
                apply_owner_effects(effects, &mut terminals);
    ...
    if let Some(request) = close_request {
        let _ = request.reply.send(Ok(CloseReport {
            timed_out,
            unresolved_requests: state.unresolved_callers(),
            joined_tasks,
            failed_tasks,
        }));
    }
    ...
    match command {
        ControlCommand::Cancel { key, now } => {
            ...
            if removed_before_dispatch || committed || !frame_cancellations.contains_key(&key) {
                let effects = state.reduce(OwnerEvent::Cancel { key, now });
                apply_owner_effects(effects, terminals);
            } else {
...
fn process_io(
    ...
    let runtime_event = match event {
        IoEvent::Inbound(frame) => Some(RuntimeEvent::InboundFrame { bytes: frame.len() }),
