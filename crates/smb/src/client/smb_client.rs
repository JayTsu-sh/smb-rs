...
impl Client {
    ...
    /// Resolves SIDs to account names using the LSARPC interface.
    ///
    /// This method opens a named pipe to `\PIPE\lsarpc`, binds to the
    /// LSARPC interface, and calls `LsarLookupSids` to translate the given SIDs
    /// into human-readable account names (e.g., `DOMAIN\Username`).
    ///
    /// ## Arguments
    ...
    ///
    /// ## Returns
    /// A `Vec<Option<TranslatedName>>` where each entry corresponds to the input SID.
    /// `None` indicates the SID could not be resolved.
    ///
    /// ## Prerequisites
    ...
    pub async fn share_connect(
        ...
        if let Ok(mchannel_map) = mchannel_map {
            ...
            self._with_connection(address.ip(), |f| {
                if let Some(session_info) = f.sessions.get_mut(&session.session_id()) {
...
impl MultiChannelUtils {
    ...
    fn get_alt_interface_addresses(
        ...
        let current_primary_interface = match network_interfaces
            .iter()
