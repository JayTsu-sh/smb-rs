...
impl ThreadingBackend {
    ...
    fn loop_send(
        ...
        send_channel: mpsc::Receiver<Option<SendFrame>>,
    ) {
        loop {
    ...
    fn loop_send_next(
        &self,
        message: Result<Option<SendFrame>, mpsc::RecvError>,
...
impl MultiWorkerBackend for ThreadingBackend {
    type SendMessage = Option<SendFrame>;
    ...
    type ChannelSender = std::sync::mpsc::SyncSender<Self::SendMessage>;
    type ChannelReceiver = mpsc::Receiver<Self::SendMessage>;
    ...
    fn stop(&self) -> crate::Result<()> {
        ...
        Self::channel_send(&self.worker.sender, None)?;

        // Join the threads.
        handles
            .0
            .join()
            .map_err(|_| Error::JoinError("Error stopping receiver".to_string()))?;
    ...
    fn make_send_channel_pair() -> (Self::ChannelSender, Self::ChannelReceiver) {
        // Bounded channel consistent with async backend's channel(100).
